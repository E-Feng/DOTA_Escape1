...
function EscapeTest:OnAbilityUsed(keys)
  ...
  if abilityname == "phase_shift_custom" then
    local abil = hero:FindAbilityByName(abilityname)
    local slot = abil:GetAbilityIndex()
...
function EscapeTest:SpawnEntities(level)
  ...
  for i,entvals in pairs(EntList[level]) do
    ...
    elseif entvals[ENT_UNTIM] == 2 then
      ...
      elseif entname == "npc_gate" then
        EscapeTest:GateThinker(unit, entvals)
      elseif entname == "npc_pudge" then
        EscapeTest:PudgeThinker(unit)
      elseif entname == "npc_th_dragon" then
        EscapeTest:DragonThinker(unit)
      elseif entname == "npc_venomancer" then
        EscapeTest:VenoThinker(unit, entvals)
      elseif entname == "npc_dota_hero_tiny" then
        EscapeTest:TinyThinker(unit, entvals)
      elseif entname == "npc_pheonix" then
        unit:RemoveSelf()
        EscapeTest:PheonixInitial(entvals)
      elseif entname == "npc_aggro" then
        EscapeTest:AggroThinker(unit, entvals)
      end
    end
  end
  if level == 3 then
    ...
    EscapeTest:SpawnParticle(give, "skillgive1", 0, "skillgive")
  elseif level == 4 then
    EscapeTest:RandomWanderUnits()
  elseif level == 5 then
    local cw = "particles/misc/sunray_cw.vpcf"
    local ccw = "particles/misc/sunray_ccw.vpcf"
    local stop = "particles/misc/sunray_stop.vpcf"
    EscapeTest:SpawnParticle(cw, "cwtrigger1", 0, "cw")
    EscapeTest:SpawnParticle(ccw, "ccwtrigger1", 0, "ccw")
    EscapeTest:SpawnParticle(stop, "phx_stop1", 0, "stop")
  elseif level == 6 then
    EscapeTest:CartyThinker()
  end
  print("----------All Entities Spawned----------")
end

-- This function spawns particles
function EscapeTest:SpawnParticle(name, loc, partcp, save)
  print("-----------Particles spawning------------")
  local entspawn = Entities:FindByName(nil, loc):GetAbsOrigin()
  local dummy = CreateUnitByName("npc_dummy_unit", entspawn, true, nil, nil, DOTA_TEAM_GOODGUYS)
  dummy:FindAbilityByName("dummy_unit"):SetLevel(1)
  local part = ParticleManager:CreateParticle(name, PATTACH_ABSORIGIN, dummy)
  ParticleManager:SetParticleControl(part, partcp, dummy:GetAbsOrigin())
  Parts[save] = part
...
function EscapeTest:HeroRevived(deadhero, alivehero)
  ...
  deadhero:RespawnHero(false, false, false)
...
function EscapeTest:PudgeThinker(unit)
...
function EscapeTest:DragonThinker(unit)
...
function EscapeTest:WanderThinker(unit)
...
function EscapeTest:PheonixInitial(entvals)
  ...
  Timers:CreateTimer(1, function()
    ...
    block1.part = Parts["cw"]
    block2.part = Parts["ccw"]
